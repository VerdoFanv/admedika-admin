...
export default function ContactListId() {
	...
	const { data: pageData, isLoading: pageIsLoading } = useGetData(`/service-inquiry/${listId}`)
	...
	async function handleDeleteSubpage() {
		...
		try {
			await deleteData(`/service-inquiry/${listId}/delete`)
		...
		pageData?.status_code === 404 ? (
			...
			<div className="admin-page">
				<header className="admin-page-header">
					<AdminHeader
						legend="Services"
						title={`${pageData?.data?.si_name}`}
						parent={<Link href="/services/inquiry">Inquiry List</Link>}
						action={
							<Access
								auth="write:services-inquiry-list"
								yes={
									...
									<div className="data-fieldset-body">
										<div className="row">
											...
											<p className="value strong">
												{pageData?.data?.si_name}
											</p>
										</div>
										<div className="row">
											<p className="label">Email</p>
											<p className="value">{pageData?.data?.si_email}</p>
										</div>
										<div className="row">
											<p className="label">Phone</p>
											<p className="value">{pageData?.data?.si_phone}</p>
										</div>
										<div className="row">
											<p className="label">Company</p>
											<p className="value">{pageData?.data?.si_company}</p>
										</div>
										<div className="row">
											<p className="label">Service</p>
											<p className="value">{pageData?.data?.si_service}</p>
										</div>
										<div className="row">
											<p className="label">Address</p>
											<p className="value value-area">{pageData?.data?.si_address}</p>
										</div>
									</div>
								</div>
							</div>
