		...
		<form onSubmit={handleSubmit(onSubmit)} className="admin-page">
			<header className="admin-page-header">
				<AdminHeader
					...
					parent={
						<>
							<Link href={breadcrumb[1]}>Pages</Link> / <Link href={breadcrumb[2]}>{ pageParentData?.data?.page_title }</Link> / <Link href={breadcrumb[3]}>{ pageData?.data?.page_title }</Link>
						</>
					}
					action={
						<Access
...
ServiceSubpageImageGalleryNew.getLayout = function getLayout(page) {
	return (
		<Dashboard>
			{ page }
		</Dashboard>
