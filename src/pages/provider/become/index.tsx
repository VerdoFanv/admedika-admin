...
export default function ProviderBecome() {
	...
	const { data: pageData, isLoading } = useGetData(`/page/${pageId[`provider-join`]}`)
	useLoading(isLoading)

	async function onSubmit(data) {
		dispatch({ type: `set_isLoading`, payload: true })
		try {
			await postData(`/page/${pageId[`provider-join`]}/update`, {
				...data,
						...
						<div className="admin-section-body admin-section-body-gray">
							<div className="form-fieldset">
								<div className="form-fieldset-body">
									...
									/>
									<div className="row">
										<FormInput setForm={setForm} name="meta.text.register_url" label="Register URL" />
									</div>
								</div>
							</div>
						</div>
