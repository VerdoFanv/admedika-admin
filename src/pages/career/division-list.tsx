...
export default function CareerDivisionList() {
	const { dispatch } = useContext(DashboardContext)
	const setForm = useForm()
	const { reset, handleSubmit } = setForm
	const { data: pageData, isLoading: pageIsLoading } = useGetData(`/option/career-division`)
	...
	const onSubmit = async (data: any) => {
		...
		try {
			await postData(`/option/career-division`, data)
		...
		<form onSubmit={handleSubmit(onSubmit)} className="admin-page">
			<header className="admin-page-header">
				<AdminHeader
					...
					action={
						<Access
							auth="write:career-division-list"
							yes={
...
CareerDivisionList.getLayout = function getLayout(page) {
	return (
		<Dashboard>
			{page}
		</Dashboard>
